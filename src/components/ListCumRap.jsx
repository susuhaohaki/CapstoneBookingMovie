...
useEffect(() => {
  if(maCumRap && cumRap.length > 0) {
    const danhSachPhim = cumRap.find(cumRap => cumRap.maCumRap === maCumRap).danhSachPhim;
    dispatch(setDanhSachPhim(danhSachPhim))
  }
},[maCumRap])
  ...
  return (
    <div className="flex h-[400px] flex-col gap-4 overflow-y-auto bg-gray-900 p-4 lg:h-[520px] lg:border-r lg:border-gray-300">
      {cumRap.map((rap,index) => {       
          ...
          key={index}
          className={
            `flex cursor-pointer items-center gap-4 rounded-lg border-b border-gray-600 p-4transition duration-300 ease-in-out hover:bg-gray-700 hover:text-orange-500 ${rap.maCumRap === maCumRap ? 'bg-gray-700 text-orange-500' : ' text-white '}`
          }
